...
import { FaUser, FaLock, FaEnvelope, FaRulerVertical, FaWeight, FaCreditCard, FaCamera, FaCheckCircle } from "react-icons/fa";
...
function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post("http://localhost:4000/api/register", userToSend, {
      ...
      if (Array.isArray(error.response?.data)) {
        ...
        setErrors(["Hubo un error al registrar. Inténtalo nuevamente."]);
      }
    }
  };
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-black text-white">
      <div className="absolute inset-0">
        <img src="/Fondo.jpg" alt="Fondo de pesas" className="w-full h-full object-cover opacity-40" />
      </div>
        ...
        <form onSubmit={handleSubmit} className="bg-gray-800 bg-opacity-60 p-6 rounded-2xl space-y-6">
          ...
          {success && (
            <div className="bg-green-500 text-white rounded-lg px-4 py-2 mb-2 text-center font-semibold flex items-center justify-center gap-2">
              <FaCheckCircle className="text-xl" /> ¡Usuario registrado correctamente! Redirigiendo...
            </div>
          )}

          {/* Notificación bonita de errores */}
          {errors.length > 0 && (
            <div className="bg-red-500 text-white rounded-lg px-4 py-2 mb-2 text-center font-semibold space-y-1">
              {errors.map((err, idx) => (
                <div key={idx}>{err}</div>
              ))}
            </div>
          ...
          <div className="text-center pt-4">
            <button type="submit" className="bg-blue-500 hover:bg-blue-600 text-white text-xl font-semibold py-3 px-8 rounded-full transition">
              INSCRIBIRSE
            </button>
          </div>
        </form>
