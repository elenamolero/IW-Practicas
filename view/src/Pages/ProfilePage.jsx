    ...
    useEffect(() => {
        const fetchMyClasses = async () => {
            try {
              const endpoint =
              user?.role === "trainer"
                ? "http://localhost:4000/api/my-upcoming-trainer-classes"
                : "http://localhost:4000/api/my-upcoming-group-classes";
    ...
    if (!user) {
      return (
        <div className="min-h-screen flex items-center justify-center pt-20">
          <p className="text-gray-500 text-lg">Cargando usuario...</p>
        </div>
      );
    }
