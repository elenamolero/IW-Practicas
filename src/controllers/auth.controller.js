...
export const deleteUser = async (req, res) => {
  try {
    ...
    res.json({ message: `User with email ${email} successfully deleted.` });
  } catch (error) {
...
export const updateUser = async (req, res) => {
  try {
    ...
    // Update user data (without modifying email or id)
    ...
    try {
      ...
      if (role === "member") {
        if (!bankAccount || !weight || !height) {
          return res.status(400).json({
            message: "Bank account, weight, and height are required fields for members."
          });
        }
      }
      ...
      if (role === "trainer") {
        if (!classesCanTeach || classesCanTeach.length === 0) {
          return res.status(400).json({
            message: "Classes that can be taught are required for trainers."
          });
        }
      }
